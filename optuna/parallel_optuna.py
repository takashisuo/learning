...
class OptunaLogisticRegression(BaseModel):
    ...
    def search_best_params(self):
        ...
        def objective(trial):
            ...
            score: np.ndarray = cross_val_score(model, self.train_x, self.train_y, cv=3)
            accuracy = score.mean()

            return accuracy
        
        # create_study: storageにsqlite3を指定。別マシンからの実行の際にstudy_nameを合致させること。load_if_existsで存在すればそこから開始する。
        # load_studyでも類似処理は可能だが, create_studyにload_if_existsを指定すればcreate/loadどちらも対応できる。
        # 精度によってmaximize/minimizeを変更する.
        # RMSEなどはminimize
        study = optuna.create_study(
            study_name="parallel-run",
